...
export const kits = (player) => {
    ...
    form.title('KITS API');
    Kits.forEach(kit => {
        if (kit.tag && !player.hasTag(kit.tag) && !player.hasTag(Script.adminTag))
            return;
        if (kit.once && player.getDynamicProperty(`ClaimedKit:${kit.name}`) && !player.hasTag(Script.adminTag))
            return;
        const lore = [
...
function splitString(text, every) {
    const result = [];
