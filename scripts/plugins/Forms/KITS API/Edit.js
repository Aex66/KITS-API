...
export const _edit = (player, kit) => {
    new ModalFormData()
        .title(`Edit ${kit.name}`)
        .textField('Description:', 'kit for vips', kit?.description)
        .textField('Tag:', 'vip', kit?.tag)
        .textField('Cooldown', '1 day 6 hours', kit?.duration)
        .textField('Price:', '500', kit?.price?.toString())
        .textField('Image:', 'minecraft:diamond', kit.image)
        .textField('Slot:', '0', kit?.slot?.toString())
        .toggle('Once', kit?.once ?? false)
        .show(player).then(res => {
        if (res.canceled)
            return kits(player);
        const ms = Date.now();
        let [desc, tag, duration, price, image, slot, onlyOnce] = res.formValues;
