import { Enchantment, Player } from "@minecraft/server"
import { ActionFormData } from "@minecraft/server-ui"

const number = 1
type ChestGuiType = 'small' | 'single' | 'large' | 'double' | '5' | '9' | '18' | '27' | '36' | '45' | '54' | 'page'
const sizes = new Map([
	...
	['page', ['§c§h§e§s§t§p§a§g§e§r', 54]]
]);
export class ChestGui {
    protected formData: { type: string, title: string, items: { data: string, icon: string | number }[] }
    constructor(){
        this.formData = {
            type: undefined,
            title: undefined,
    ...
    setType(type: ChestGuiType): ChestGui {
		this.formData['type'] = type;
        const size = sizes.get(type) ?? ['§c§h§e§s§t§2§7§r', 27]; //@ts-ignore
		this.formData.title = size[0]//@ts-ignore
        for (let i = 0; i < size[1]; i++)
            this.formData.items.push({ data: '', icon: undefined });
        return this;
    }
    title(title: string): ChestGui {
		this.formData.title += title
        return this;
    }
    item(slot: number, name: string, lore: string[], icon?: string, stackSize = 1, enchanted = false, enchants?: Enchantment[]): ChestGui {
		let replaces = {
			'neapolis:bill': 'minecraft:paper',
			'ranks:ranger': 'minecraft:paper',
			'ranks:shaman': 'minecraft:paper',
			'ranks:sage': 'minecraft:paper',
			'ranks:wizard': 'minecraft:paper',
			'ranks:noble': 'minecraft:paper',
			'ranks:maradona': 'minecraft:paper',
			'ranks:vip': 'minecraft:paper',
		}
		//@ts-ignore
		if (icon in replaces) icon = replaces[icon]
		const ID = typeIdToDataId.get(icon.startsWith('minecraft:') ? icon : 'minecraft:' + icon) ?? typeIdToID.get(icon.startsWith('minecraft:') ? icon : 'minecraft:' + icon);
		let enchantments = enchants?.map(e => `§7${beauty('minecraft:' + e.type)} §e${toRomanNumeral(e.level)}`)
		//@ts-ignore
		this.formData.items.splice(slot, 1, { data: `stack#${Math.min(Math.max(stackSize, 1) || 1, 99).toString().padStart(2, '0')}§r${name ?? ''}§r${enchantments?.length ? `\n§r${enchantments.join('\n§r')}` : ''}§r${lore?.length ? `\n§r${lore.join('\n§r')}` : ''}`, icon: (((ID + (ID < 256 ? 0 : number)) * 65536) + (!!enchanted * 32768)) || icon })
		return this
    }
	pattern(pattern: string[], key: { [key: string]: { data: { name: string, lore: string[], enchanted?: boolean, stackSize?: number, enchants?: Enchantment[] }, icon: string } }) {
        for (let i = 0; i < pattern.length; i++) {
            ...
            for (let j = 0; j < row.length; j++) {
                ...
                if (key[letter]) {
                    ...
                    const icon = key[letter]?.icon;
                    const ID = typeIdToDataId.get(icon.startsWith('minecraft:') ? icon : 'minecraft:' + icon) ?? typeIdToID.get(icon.startsWith('minecraft:') ? icon : 'minecraft:' + icon);
                    let enchantments = [];
					const enchants = data.enchants || []
                    enchantments = enchants.map(e => `§7${beauty('minecraft:' + e.type.toString())} §e${toRomanNumeral(e.level)}§r`);
					const lore = data?.lore?.length ? `\n§r${data.lore?.join('\n§r')}` : ''
					//@ts-ignore
                    this.formData.items.splice(slot, 1, { data: `stack#${Math.min(Math.max(data?.stackSize??1, 1) || 1, 99).toString().padStart(2, '0')}§r${data.name ?? ''}§r${enchantments?.length ? `\n§r${enchantments.join('\n§r')}` : ''}§r${lore}`, icon: (((ID + (ID < 256 ? 0 : number)) * 65536) + (!!data?.enchanted * 32768)) || icon })
                }
            }
        }
        return this;
	...
	getEmptySlots() {
		const slots = []
		const size = sizes.get(this.formData.type) ?? ['§c§h§e§s§t§2§7§r', 27]; //@ts-ignore
		for (let i = 0; i<size[1]; i++) {
    ...
    show(player: Player) {
        //yEAh
        if (!this.formData?.type || !this.formData?.items?.length)
            throw Error('ChestGui is missing data!');
        const form = new ActionFormData();
        form.title(this.formData.title);

        this.formData.items.forEach((item) => form.button(item.data, item.icon?.toString()));
...
export function beauty(text: string) {
